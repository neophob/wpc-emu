...
class Emulator {
  ...
  getUiState() {
    ...
    const asicChangedState = this.uiFacade.getChangedState(uiState.asic);
  ...
  executeToCycle(targetTicks) {
    return this.cpuBoard.executeToCycle(targetTicks);
  }
  ...
  setDirectInput(type, uint8Value, offset) {
    this.cpuBoard.setDirectInput(type, uint8Value, offset);
  }
