...
function getConcatinatedSoundRoms(uInt8Roms) {
  if (!uInt8Roms.u18 || !uInt8Roms.u15 || !uInt8Roms.u14) {
 ...
 */
function findSecurityGameIdInRom(gameRom, systemRom) {
  console.log('SEARCH GAME ID IN ROM');
  let foundGameIdLink = 0;
  for (let x = 0; x < gameRom.length - 7; x++) {
    const prefixMatches = gameRom[x] === 0xEC && gameRom[x + 1] === 0x9F;
    if (prefixMatches) {
      const suffixMatches = gameRom[x + 4] === 0x83 && gameRom[x + 5] === 0x12 && gameRom[x + 6] === 0x34;
      if (suffixMatches && ++foundGameIdLink === 2) {
        ...
        console.log('FOUND GAMEID LOCATION:', gameIdMemoryLocation);
...
function parse(uInt8Roms, metaData = {}) {
  return new Promise((resolve, reject) => {
    ...
    const gameIdMemoryLocation = findSecurityGameIdInRom(gameRom, systemRom);
