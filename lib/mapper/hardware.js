...
$2000-$3FFF (8 KiB)	Hardware
            ...
            $2000-$37FF	    Expansion (maybe security chip of WPC-S and WPC-95)
                            TODO i guess DMD Pages is different
...
const MEMORY_ADDR_LOW_UNSUPPORTED = 0x2000;
const MEMORY_ADDR_EXPANSION = 0x3800;
const MEMORY_ADDR_DMD = 0x3FC0;
const MEMORY_ADDR_EXTERNAL_IO = 0x3FE0;
const MEMORY_ADDR_WPC_IO = 0x4000;


const SUBSYSTEM_WPCIO = 'wpcio';
const SUBSYSTEM_EXTERNALIO = 'externalio';
const SUBSYSTEM_DMD = 'dmd';

module.exports = {
  getAddress,
  SUBSYSTEM_WPCIO,
  SUBSYSTEM_EXTERNALIO,
  SUBSYSTEM_DMD,
};
...
function getAddress(offset) {
  if (offset < MEMORY_ADDR_LOW_UNSUPPORTED) {
    throw new Error('HW_GET_ADDRESS_INVALID_MEMORY_REGION_0x' + offset.toString(16));
  }
  if (offset < MEMORY_ADDR_EXPANSION) {
    return buildReturnModel(offset, SUBSYSTEM_EXTERNALIO);
  }
  if (offset < MEMORY_ADDR_DMD) {
    return buildReturnModel(offset, SUBSYSTEM_DMD);
  }
