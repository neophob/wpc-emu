...
onmessage = (event) => {
  ...
  switch (message) {
    ...
    case MESSAGE.executeCycles:
      if (emuState) {
        // do not execute cycle, to avoid skipped frames
        console.log('WARN_FRAME_NOT_FETCHED_YET');
        return;
      }
      emu.executeCycle();
      emuState = emu.getUiState();
      break;
...
class Emu {
  ...
  configureFramerate(frameRate) {
    if (frameRate < 1 || frameRate > 200) {
      console.error('INVALID_FRAMERATE', frameRate);
      return;
    }
  ...
  executeCycle() {
    return this.emuInstance.executeCycle(this.ticksPerCall, TICKS_PER_STEP);
  }
