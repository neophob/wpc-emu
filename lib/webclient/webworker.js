...
function handleMessage(event, postMessage) {
  ...
  if (message === MESSAGE.initializeEmulator) {
    return WebworkerEmu.buildWpcInstance(parameter.romData, parameter.gameEntry)
      ...
      .catch((error) => {
        postMessage({ message: MESSAGE.MSG_TYPE_ERROR, requestId, parameter: error.message });
      });
  }
  ...
  switch (message) {
    ...
    case MESSAGE.setSwitchInput:
      const optionalSwitchInputParameter = event.data.length > 3 ? event.data[3] : undefined;
      emu.setSwitchInput(parameter, optionalSwitchInputParameter);
      postMessage({ message: MESSAGE.MSG_TYPE_ACK, requestId, parameter: optionalSwitchInputParameter });
      break;

    case MESSAGE.setFliptronicsInput:
      const optionalFliptronicsParameter = event.data.length > 3 ? event.data[3] : undefined;
      emu.setFliptronicsInput(parameter, optionalFliptronicsParameter);
      postMessage({ message: MESSAGE.MSG_TYPE_ACK, requestId, parameter: optionalFliptronicsParameter });
      break;
    ...
    case MESSAGE.setDipSwitchByte:
      if (!parameter) {
        return postMessage({ message: MESSAGE.MSG_TYPE_ERROR, requestId, parameter: 'MISSING_DIPSWITCH_PARAMETER' });
      }
      emu.setDipSwitchByte(parameter);
      postMessage({ message: MESSAGE.MSG_TYPE_ACK, requestId });
      break;

    case MESSAGE.getDipSwitchByte:
      const dipSwitchByte = emu.getDipSwitchByte();
      postMessage({ message: MESSAGE.MSG_TYPE_ACK, requestId, parameter: dipSwitchByte });
      break;
    ...
    case MESSAGE.setEmulatorState:
      if (!parameter) {
        return postMessage({ message: MESSAGE.MSG_TYPE_ERROR, requestId, parameter: 'MISSING_STATE_PARAMETER' });
      }
    ...
    case MESSAGE.configureFrameRate:
      if (!Number.isInteger(parameter)) {
        return postMessage({ message: MESSAGE.MSG_TYPE_ERROR, requestId, parameter: 'INVALID_FRAMERATE_PARAMETER_' + parameter });
      }
