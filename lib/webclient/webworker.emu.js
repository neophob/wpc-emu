...
class Emu {
  ...
  constructor(emuInstance) {
    ...
    this.ticksPerCall = Number.parseInt(TICKS_PER_SECOND / this.desiredFrameRate, 10);
  }
  ...
  setSwitchInput(switchNr, optionalValue) {
    this.emuInstance.setSwitchInput(switchNr, optionalValue);
  }

  setFliptronicsInput(value, optionalValue) {
    this.emuInstance.setFliptronicsInput(value, optionalValue);
  }
  ...
  setDipSwitchByte(dipSwitch) {
    this.emuInstance.setDipSwitchByte(dipSwitch);
  }

  getDipSwitchByte() {
    return this.emuInstance.getDipSwitchByte();
  }
  ...
  emuStepByTime(advanceByMs, ticksStep) {
    this.emuInstance.emuStepByTime(advanceByMs, ticksStep);
  }
  ...
  writeMemory(offset, value) {
    this.emuInstance.cpuBoard.writeMemory(offset, value);
  }
