...
const DEFAULT_PENDING_ANSWERS = 32;
const COMM_TIMEOUT_MS = 1000;

function buildRpcProxy(maximalPendingAnswers) {
  return new RpcProxy(maximalPendingAnswers);
}

class RpcProxy {

  constructor(maximalPendingAnswers = DEFAULT_PENDING_ANSWERS) {
   ...
   */
  waitOnAnswer(requestId, timeoutMs = COMM_TIMEOUT_MS) {
    ...
    if (this.pendingAnswers.has(requestId)) {
      console.debug('REMOVE DUPLICATE REQUEST');
