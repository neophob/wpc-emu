...
const BoardExternalIo = require('./boards/external-io.js');
...
class WpcAsic {
  ...
  constructor(romObject, interruptCallback) {
    ...
    if (Array.isArray(romObject.switchesEnabled)) {
      romObject.switchesEnabled.forEach((switchIdToEnable) => {
        console.log('INIT::enable switch', switchIdToEnable);
        this.setCabinetInput(switchIdToEnable);
      });
    }
  }
  ...
  executeCycle(ticks) {
    //console.log('clear');
    this.boardWpc.executeCycle(ticks);
  }
  ...
  write8(offset, value) {
    ...
    switch (address.subsystem) {
      ...
      default:
        throw new Error('INVALID_WRITE_SUBSYSTEM');
    }
  }
