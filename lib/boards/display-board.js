...
class DisplayBoard {
  ...
  getState() {
    if (this.hasAlphanumericDisplay) {
      return {
        segmentColumn: this.outputAlphaDisplay.segmentColumn,
        dmdShadedBuffer: this.outputAlphaDisplay.getShadedOutputVideoFrame(),
      };  
    }
  ...
  setState(displayState) {
    //TODO    if (this.hasAlphanumericDisplay) {
  ...
  executeCycle(singleTicks) {
    ...
    if (this.ticksUpdateAlpha >= 512*timing.CALL_WPC_UPDATE_DISPLAY_AFTER_TICKS) {
      this.outputAlphaDisplay.clear();
      this.ticksUpdateAlpha -= 512*timing.CALL_WPC_UPDATE_DISPLAY_AFTER_TICKS;
    }
