...
class UiState {
      ...
      .map((entry) => {
        switch (entry.type) {
          ...
          case ENCODING_BCD:
            ...
            const number = bcd.toNumber(ram.subarray(entry.offset, entry.offset + bcdLength));
            entry.value = number;
            break;
  ...
  getChangedAsicState(state, includeExpensiveData = true) {
    ...
    const videoRamChanged = includeExpensiveData === false ? false : this.getVideoRamDiff(state.display.videoRam);
    const memoryPosition = this.parseMemoryPosition(state.ram);
    const ram = includeExpensiveData === false ? false : state.ram;
    return {
      ram,
      memoryPosition,
      sound: state.sound,
      ...
      dmd: {
        ...
        lastVideoCaptureTimeTicks: state.display.lastVideoCaptureTimeTicks,
      },
    };
  }
