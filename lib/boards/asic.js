...
const SYSTEM_ROM_BANK_NUMBER = [62, 63];

module.exports = {
  getInstance,
  SYSTEM_ROM_BANK_NUMBER,
  OP,
};
  ...
  write(offset, value) {
    ...
    switch (offset) {
      ...
      case OP.WPC_ROM_BANK:
        ...
        const normalisedBank = bank + (0x3F ^ this.pageMask);
        const systemRomRead = SYSTEM_ROM_BANK_NUMBER.includes(normalisedBank);
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_CLK_HOURS_DAYS: {
        ...
        checksum += this.ram[0x1803] = temp.getDate(); //day of the month
        checksum += this.ram[0x1804] = temp.getDay() + 1; //days since Sunday
