...
class CpuBoardAsic {
  ...
  setMidnightMadnessMode(boolean) {
    this.midnightModeEnabled = boolean === true;
  }
  ...
  getTime() {
    ...
    const tempTime = new Date('December 17, 1995 23:59:45');
    return new Date(tempTime.getTime() + Date.now() - this.startTime);
  }
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_CLK_HOURS_DAYS: {
        ...
        let checksum = 0;
        //Starts with a two-byte year (2015 is 0x07 0xDF),
        checksum += this.ram[0x1800] = temp.getFullYear() >> 8;
        checksum += this.ram[0x1801] = temp.getFullYear() & 0xFF;
        //month (1-12),
        checksum += this.ram[0x1802] = temp.getMonth() + 1;
        //day of month (1-31)
        checksum += this.ram[0x1803] = temp.getDate();
        //day of the week (0-6, 0=Sunday)
        checksum += this.ram[0x1804] = temp.getDay() + 1;
        //hour (0-23)
        checksum += this.ram[0x1805] = 0;
        //minute (0-59)
        checksum += this.ram[0x1806] = 1;
        checksum = 0xFFFF - checksum;
        this.ram[0x1807] = checksum >> 8;
        this.ram[0x1808] = checksum & 0xFF;
