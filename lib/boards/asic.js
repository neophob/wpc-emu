...
const SERIALISED_STATE_VERSION = 2;
...
class CpuBoardAsic {
  ...
  getState() {
    ...
    return {
      version: SERIALISED_STATE_VERSION,
  ...
  setState(wpcState = {}) {
    if (wpcState.version !== SERIALISED_STATE_VERSION) {
      console.log('INVALID_STATE_VERSION');
      return false;
    }
  ...
  executeCycle(ticksExecuted) {
    ...
    //console.log('this.watchdogTicks',this.watchdogTicks)
    if (this.watchdogTicks < 0) {
      console.log('ERRRRRRRRRRR', this.watchdogTicks)
    }

    //2000000/1000/64 -> decrease watchdog after 31 ticks
  //  const watchdogSteps = parseInt(ticksExecuted / 31, 10);
  //  this.watchdogCount -= watchdogSteps;
  ...
  write(offset, value) {
    ...
    switch (offset) {
      ...
      case OP.WPC_ROM_BANK: {
        ...
        const normalizedBank = bank + (MAXIMAL_BANK_NO ^ this.pageMask);
        const systemRomRead = SYSTEM_ROM_BANK_NUMBERS.includes(normalizedBank);
        ...
        if (systemRomRead) {
          debug('SELECT SYSTEM WPC_ROM_BANK', { value, normalizedBank, bank });
          this.romBank = normalizedBank;
          return;
        }
      ...
      case OP.WPC_ZEROCROSS_IRQ_CLEAR: {
        debug('WRITE', REVERSEOP[offset], value);
         ...
         if (value & WPC_ZC_WATCHDOG_RESET) {
          ...
          console.log('WRITE WPC_ZEROCROSS_IRQ_CLEAR: RESET WATCHDOG');
          this.watchdogTicks = 2049 * 2; // + 1261
        }
        ...
        if (value & WPC_ZC_IRQ_CLEAR) {
          ...
          console.log('WPC_ZC_IRQ_CLEAR',this.watchdogTicks);
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_SW_JUMPER_INPUT:
        ...
        debug('READ WPC_SW_JUMPER_INPUT', JUMPER_SETTINGS);
