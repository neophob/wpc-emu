...
class DMD {
  ...
  setState(dmdState) {
    ...
    this.dmdPageMapping = dmdState.dmdPageMapping;
    this.activepage = dmdState.activepage;
    this.videoRam = Uint8Array.from(dmdState.videoRam);
    this.videoOutputPointer = dmdState.videoOutputPointer;
    this.requestFIRQ = dmdState.requestFIRQ;
  }
  ...
  copyScanline() {
    ...
    if (this.scanline === 0) {
      ...
      //TODO:
      // TODO trigger this at a 60hz rate!
      // - STTNG (SY 3.08, 2/17/94)
      // - JD (SY 3.08, 12/29/93)
      // - DM (SY 3.21, 5/19/94)
      // - PSTE (SY 3.14, 2/14/94)
      //do not display anything when requestFIRQ is used! looks like a DCS specific issue. however
      // - IJ (SY 3.03, 11/22/93)
      // - NOTE: using an older JD (SY 2.67, 08/19/93) shows the same issues, so it might not be related to the SY version

//      if (this.requestFIRQ) {
//        this.requestFIRQ = false;
        if (this.ram[OP.WPC_DMD_SCANLINE] > 0 && this.ram[OP.WPC_DMD_SCANLINE] < 31) {
