...
//TODO mapper
const PAGE_OFFSET = [0x3800, 0x3A00, 0x3000, 0x3200, 0x3400, 0x3600];
...
class DMD {
  ...
  _getPageOffset(bank, offset) {
    const delta = offset - PAGE_OFFSET[bank];
    const selectedPage = this.dmdPageMapping[bank];
    return selectedPage * DMD_PAGE_SIZE + delta;
  }
  ...
  write(offset, value) {
    var _ofs;
    if (offset < DMD_START_PAGE_1_ADDRESS) {
      //TODO implement other cpu mappings (0x3000 - 0x3600)
      console.log('NOT_IMPLEMENTED!',offset, value)
    }
    if (offset < DMD_START_PAGE_2_ADDRESS) {
      debug('WRITE DMD_PAGE_LOW', value);
      _ofs = this._getPageOffset(0, offset);
      this.videoRam[_ofs] = value;
      return;
    }
    if (offset < DMD_END_PAGE_2_ADDRESS) {
      debug('WRITE DMD_PAGE_HIGH', value);
      _ofs = this._getPageOffset(1, offset);
      this.videoRam[_ofs] = value;
      return;
    }
