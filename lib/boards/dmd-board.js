...
class DMD {
  ...
  copyScanline() {
    ...
    // NOTE: if this.ram[OP.WPC_DMD_SCANLINE] > 0x1F then NO FIRQ call is made
    if (this.requestFIRQ && this.scanline === this.ram[OP.WPC_DMD_SCANLINE] && this.scanline <= DMD_MAXIMAL_SCANLINE) {
      if (this.interruptCallback.firqFromDmd()) {
        // clear the FIRQ flag only if FIRQ could be triggered - else STTNG does not output anything
