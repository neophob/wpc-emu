  ...
  update(value, irqCountGI = 0) {
    ...
    for (let i = 0; i < 5; i++) {
      if (normalizedValue & (1 << i)) {
        if (this.generalIlluminationState[i] < 7) {
          this.generalIlluminationState[i] = 7 - irqCountGI;
        } else {
          this.generalIlluminationState[i] = 7;
        }
      } else if (this.generalIlluminationState[i]) {
          this.generalIlluminationState[i]--;
        }
    }

    this.generalIlluminationState[COIN_DOOR_ENABLE_BIT] = (normalizedValue & (1 << COIN_DOOR_ENABLE_BIT)) ? 7 : 0;
    this.generalIlluminationState[UNKNOWN_BIT] = (normalizedValue & (1 << UNKNOWN_BIT)) ? 7 : 0;
    this.generalIlluminationState[FLIPPER_RELAY_BIT] = (normalizedValue & (1 << FLIPPER_RELAY_BIT)) ? 7 : 0;
  }
