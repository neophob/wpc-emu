...
class SecurityPIC {
//DH 232/0xE8 -> 530/0x212
//MM 131/0x83 -> 559/0x22F
//df 101 -> 29
  ...
  _getCalculatedSecurityCode() {
    const expectedUnlockCode = ((this.machineNumber >> 8) * (0x100 * this.gameSerialNumber[14] + this.gameSerialNumber[16])) +
  		((this.machineNumber & 0xFF) * (0x100 * this.gameSerialNumber[15] + this.gameSerialNumber[14]));
...
/*                         pic_unlock_code[0] = (reg >> 16) & 0xFF;
    ...
    const a = expectedUnlockCode>>16;
    const b = (expectedUnlockCode>>8)&0xff;
    const c = expectedUnlockCode&0xff;
    console.log('1', a);
    console.log('2', b);
    console.log('3', c);*/
    return expectedUnlockCode & 0xFFFFFF;
...
*/
  ...
  reset() {
    ...
    this.unlockMode = 0;
    this.unlockCode = 0;
    this.expectedUnlockCode = 0;//this._getCalculatedSecurityCode();
    this.writesUntilUnlockNeeded = 0x20;
    this.alreadyWarned = false;
    console.log('RESET SECURITY PIC', this.expectedUnlockCode);
  }
  ...
  write(data) {
    ...
    if (this.unlockMode > 0) {
      ...
      console.log('unlockCode', this.unlockCode, data);

			if (++this.unlockMode > 3) {
        if (!this.expectedUnlockCode) {
          // learn mode
          this.expectedUnlockCode = this.unlockCode;
          console.log('LEARN MODE', this.expectedUnlockCode, this.unlockCode);
        }
				if (this.unlockCode !== this.expectedUnlockCode) {
					if (!this.alreadyWarned) {
            debug('Invalid PIC unlock code', { actual: this.unlockCode, expected: this.expectedUnlockCode });
            console.log('Invalid PIC unlock code', { actual: this.unlockCode, expected: this.expectedUnlockCode });
