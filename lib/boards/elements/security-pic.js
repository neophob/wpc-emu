...
class SecurityPIC {
  ...
  _init() {
    this.lastByteWrite = 0xFF;
    this.unlockMode = 0;
    this.unlockCode = 0;
    this.expectedUnlockCode = 0;
    this.writesUntilUnlockNeeded = 1000;
  ...
  read(getActiveRowFunction) {
      ...
      case this._readSwichColumn(7):
				...
				return getActiveRowFunction()[col + 1];
      ...
      case this._readPicSerial(15):
				return this.serialNumber[this.lastByteWrite - this._readPicSerial(0)];
  ...
  write(data) {
    /* Handles writes to the PIC */
		if (this.lastByteWrite === 0xFF && data !== WPC_PIC_RESET) {
			debug('PIC write %02X before reset.', data);
      return;
		}
    ...
    if (this.unlockMode > 0) {
            ...
            debug('Invalid PIC unlock code %X (expected %X)', this.unlockMode, this.expectedUnlockCode);
