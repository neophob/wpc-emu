const debug = require('debug')('wpcemu:boards:elements:soundSerialInterface');
...
class SoundInterface {
  ...
  writeData(value) {
    debug('DATA_WRITE', value);
    this.writeQueue.insert(value);

    if (this.isPreDcsSoundBoard) {
      this._processPreDcsSoundCommand();
    } else {
      this._processDcsSoundCommand();
    }
  }

  readControl() {
    if (this.readQueue.isEmpty()) {
      debug('READ_CONTROL_EMPTY');
      return READ_CONTROL_NO_DATA_IS_AVAILABLE;
    }
    debug('READ_CONTROL_AVAILABLE');
    return READ_DATA_IS_AVAILABLE;
  }

  writeControl(value) {
    debug('CONTROL_WRITE', value);
    if (value === 0x00) {
    ...
    if (value !== this.lastUnknownControlWrite) {
      console.log('SND: UNKNOWN CONTROL WRITE:', value);
