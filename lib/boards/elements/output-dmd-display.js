...
function getInstance() {
  return new OutputDmdDisplay();
}

class OutputDmdDisplay {
  constructor() {
    this.dmdPageSize = DMD_PAGE_SIZE;
    this.shadedVideoBuffer = new Uint8Array(DMD_PAGE_SIZE * DMD_SHADING_FRAMES).fill(0);
  ...
  writeVideoRam(bank, offset, value) {
    const videoRamOffset = this._getPageOffset(bank, offset);
    ...
    for (let scanline = 0; scanline < DMD_WINDOW_HEIGHT; scanline++) {
      for (let eightPixelsOffset = 0; eightPixelsOffset < DMD_WINDOW_WIDTH_IN_BYTES; eightPixelsOffset++) {
        for (let i = 0; i < 8; i++) {
          ...
          const intensity = ((this.shadedVideoBuffer[OFFSET_BUFFER0 + inputOffset] & bitMask) > 0 ? 1 : 0) +
                            ((this.shadedVideoBuffer[OFFSET_BUFFER1 + inputOffset] & bitMask) > 0 ? 1 : 0) +
                            ((this.shadedVideoBuffer[OFFSET_BUFFER2 + inputOffset] & bitMask) > 0 ? 1 : 0);
