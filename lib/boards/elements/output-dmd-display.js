...
class OutputDmdDisplay {
  ...
  updateVideoBuffer(videoBuffer) {
    // clone video buffer
    this.videoBuffer = Uint8Array.from(videoBuffer);
  }
  ...
  getShadedOutputVideoFrame() {
    const BUFFER0 = this.dmdPageSize * 0;
    const BUFFER1 = this.dmdPageSize * 1;
    const BUFFER2 = this.dmdPageSize * 2;
    // output is 8 times larger (1 pixel use 1 byte)
