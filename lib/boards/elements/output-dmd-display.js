...
class OutputDmdDisplay {
  constructor(dmdPageSize) {
    ...
    this.lastVideoCaptureTimeTicks = 0;
  }
  ...
  getState() {
    return {
      ...
      lastVideoCaptureTimeTicks: this.lastVideoCaptureTimeTicks,
    };
  }
  ...
  writeVideoRam(bank, offset, value) {
    ...
    debug('WR VID %o', {bank,offset,value});
    ...
    if (this.scanline === 0) {
      ...
      if (this.videoOutputPointer === 0) {
        if (!this.arraysEqual(this.referenceVideoBuffer, this.shadedVideoBufferLatched)) {
          this.lastVideoCaptureTimeTicks = this.videoTicksCounter;
          this.referenceVideoBuffer = this.shadedVideoBufferLatched;
        }
      }
