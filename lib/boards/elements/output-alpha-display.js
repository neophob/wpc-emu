...
const OFFSET_BOTTOM = OFFSET_LINE * CHAR_HEIGHT;
...
*/
  ...
  if (value & SEG_VERT_BOT) {
    const EIGHT_LINES = OFFSET_LINE * 8;
  ...
  getShadedOutputVideoFrame() {
    ...
    for (let n = 0; n < 16; n++) {
      this._drawChar(videoBufferShaded, n * CHAR_WITH, 0xffff);//this.displayData[n]);
    }
    for (let n = 16; n < 32; n++) {
      this._drawChar(videoBufferShaded, OFFSET_BOTTOM + n * CHAR_WITH, 0xffff);//this.displayData[n]);
    }
