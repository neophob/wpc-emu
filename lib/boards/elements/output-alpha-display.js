...
class OutputAlphaDisplay {
  ...
  _drawChar(buffer, offset, value) {
    ...
    if (value & SEG_MIDDLE_RIGHT) {
      buffer[OFFSET_MIDDLE + 2 + offset] = 3;
      buffer[OFFSET_MIDDLE + 3 + offset] = 3;
      buffer[OFFSET_MIDDLE + 4 + offset] = 3;
    }  
    if (value & SEG_VERT_TOP) {
      buffer[offset + 2] = 3;
      buffer[offset + 2 + 1 * OFFSET_LINE] = 3;
      buffer[offset + 2 + 2 * OFFSET_LINE] = 3;
      buffer[offset + 2 + 3 * OFFSET_LINE] = 3;
      buffer[offset + 2 + 4 * OFFSET_LINE] = 3;
      buffer[offset + 2 + 5 * OFFSET_LINE] = 3;
    }
    if (value & SEG_VERT_BOT) {
      const SIX_LINES = OFFSET_LINE * 5;
      buffer[SIX_LINES + offset + 2] = 3;
      buffer[SIX_LINES + offset + 2 + 1 * OFFSET_LINE] = 3;
      buffer[SIX_LINES + offset + 2 + 2 * OFFSET_LINE] = 3;
      buffer[SIX_LINES + offset + 2 + 3 * OFFSET_LINE] = 3;
      buffer[SIX_LINES + offset + 2 + 4 * OFFSET_LINE] = 3;
      buffer[SIX_LINES + offset + 2 + 5 * OFFSET_LINE] = 3;
    }
    ...
    if (value & SEG_LOWER_RIGHT) {
      buffer[4 + OFFSET_MIDDLE + offset] = 3;
      buffer[4 + OFFSET_MIDDLE + offset + 1 * OFFSET_LINE] = 3;
      buffer[4 + OFFSET_MIDDLE + offset + 2 * OFFSET_LINE] = 3;
      buffer[4 + OFFSET_MIDDLE + offset + 3 * OFFSET_LINE] = 3;
      buffer[4 + OFFSET_MIDDLE + offset + 4 * OFFSET_LINE] = 3;
      buffer[4 + OFFSET_MIDDLE + offset + 5 * OFFSET_LINE] = 3;
    }
  ...
  getShadedOutputVideoFrame() {
    ...
    for (let n = 16; n < 32; n++) {
      this._drawChar(videoBufferShaded, OFFSET_LINE * 16 + n * CHAR_WITH, this.displayData[n]);
    }
