...
function buildReturnModel(offset, subsystem, page) {
  return { offset, subsystem, page };
}
...
function getAddress(offset) {
  ...
  if (offset < MEMORY_ADDR_DMD_PAGE3200) {
    return buildReturnModel(offset - 0x3000, SUBSYSTEM_DMD_VIDEORAM, 2);
  }
  if (offset < MEMORY_ADDR_DMD_PAGE3400) {
    return buildReturnModel(offset - 0x3200, SUBSYSTEM_DMD_VIDEORAM, 3);
  }
  if (offset < MEMORY_ADDR_DMD_PAGE3600) {
    return buildReturnModel(offset - 0x3400, SUBSYSTEM_DMD_VIDEORAM, 4);
  }
  if (offset < MEMORY_ADDR_DMD_PAGE_LOW) {
    return buildReturnModel(offset - 0x3600, SUBSYSTEM_DMD_VIDEORAM, 5);
  }
  if (offset < MEMORY_ADDR_DMD_PAGE_HIGH) {
    return buildReturnModel(offset - 0x3800, SUBSYSTEM_DMD_VIDEORAM, 0);
  }
  if (offset < MEMORY_ADDR_DMD_PAGE_HIGH + 0x200) {
    return buildReturnModel(offset - 0x3A00, SUBSYSTEM_DMD_VIDEORAM, 1);
  }
