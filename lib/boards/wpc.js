...
require('./lib/performancenow-shim.js');
...
function getInstance(romSizeMBit, interruptCallback, ram) {
  return new WpcIo(romSizeMBit, interruptCallback, ram);
}


class WpcIo {
  constructor(romSizeMBit, interruptCallback, ram) {
    this.interruptCallback = interruptCallback;
    this.pageMask = PAGESIZE_MAP[romSizeMBit];
    debug('pageMask calculated %o', { pageMask: this.pageMask, romSizeMBit });
    this.hardwareRam = ram;
  ...
  getUiState() {
    const diagnosticLed = this.hardwareRam[OP.WPC_LEDS];
    const lampRow = this.hardwareRam[OP.WPC_LAMP_ROW_OUTPUT];
    const lampColumn = this.hardwareRam[OP.WPC_LAMP_COL_STROBE];
    const switchRow = this.hardwareRam[OP.WPC_SWITCH_ROW_READ];
    const switchColumn = this.hardwareRam[OP.WPC_SWITCH_COL_SELECT];
  ...
  setCabinetInput(value) {
    ...
    this.hardwareRam[OP.WPC_SW_CABINET_INPUT] = value & 0xff;
  }
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_EXTBOARD2:
      case OP.WPC_EXTBOARD3:
        debug('READ', REVERSEOP[offset], this.hardwareRam[offset]);
        return this.hardwareRam[offset];

      case OP.WPC_SW_CABINET_INPUT:
        debug('READ AND CLEAR', REVERSEOP[offset], this.hardwareRam[offset]);
        temp = this.hardwareRam[offset];
        this.hardwareRam[offset] = 0x00;
        return temp;

      case OP.WPC_ROM_BANK:
        debug('READ', REVERSEOP[offset], this.hardwareRam[offset]);
        return this.hardwareRam[offset] & this.pageMask;

      case OP.WPC_SW_ROWREAD:
        debug('READ', REVERSEOP[offset], this.hardwareRam[offset]);
        //TODO
        return this.hardwareRam[OP.WPC_SWITCH_COL_SELECT];

      case OP.WPC_SHIFTADDRH:
        temp = this.hardwareRam[OP.WPC_SHIFTADDRH] + ((this.hardwareRam[OP.WPC_SHIFTADDRL] + (this.hardwareRam[OP.WPC_SHIFTBIT] >>> 3)) >>> 8);
        debug('READ WPC_SHIFTADDRH', temp);
        return temp;
      case OP.WPC_SHIFTADDRL:
        temp = (this.hardwareRam[OP.WPC_SHIFTADDRL] + (this.hardwareRam[OP.WPC_SHIFTBIT] >>> 3)) & 0xff;
        debug('READ WPC_SHIFTADDRL', temp);
        return temp;
      case OP.WPC_SHIFTBIT:
        debug('READ WPC_SHIFTBIT');
        return 1 << (this.hardwareRam[OP.WPC_SHIFTBIT] & 0x07);
      case OP.WPC_SHIFTBIT2:
        debug('READ WPC_SHIFTBIT2');
        return 1 << (this.hardwareRam[OP.WPC_SHIFTBIT2] & 0x07);
      ...
      case OP.WPC_ZEROCROSS_IRQ_CLEAR:
        temp = this.zeroCrossFlag << 7 | (this.hardwareRam[OP.WPC_ZEROCROSS_IRQ_CLEAR] & 0x7F);
