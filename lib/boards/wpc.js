...
function getInstance(romSizeMBit, initObject) {
  return new WpcIo(romSizeMBit, initObject);
}


class WpcIo {
  constructor(romSizeMBit, initObject) {
    ...
    this.periodicIRQTimerEnabled = undefined;
    this.romBank = 0;
    this.diagnosticLedToggleCount = 0;
    this.oldDiagnostigLedState = 0;

    this.activeLampRow = 0;
    this.activeLampCol = 0;
  }

  getUiState() {
    const diagnosticLed = this.ram[OP.WPC_LEDS];
    const lampRow = this.ram[OP.WPC_LAMP_ROW_OUTPUT];
    const lampColumn = this.ram[OP.WPC_LAMP_COL_STROBE];
    const switchRow = this.ram[OP.WPC_SWITCH_ROW_SELECT];
    const switchColumn = this.ram[OP.WPC_SWITCH_COL_SELECT];

    return {
      diagnosticLed,
      lampRow,
      lampColumn,
      lampActive: this.activeLamp,
      switchRow,
      switchColumn,
  ...
  setCabinetInput(value) {
    ...
    this.ram[OP.WPC_SWITCH_CABINET_INPUT] = value & 0xff;
  }

  irqSourceDmd(fromDmd) {
    debug('irqSourceDmd', fromDmd);
    this._irqSourceDmd = fromDmd;
  }

  _updateActiveLamp(row, column) {
    const activeLampRow = bitmagic.findMsbBit(row);
    const activeLampCol = bitmagic.findMsbBit(column);

    // HINT: when row or column matrix line is off, no lamp is on
    // value 0 means no lamp is on, value 1 .. 64 means this lamp is on
    if (!activeLampRow || !activeLampCol) {
      this.activeLamp = 0;
    } else {
      this.activeLamp = (activeLampRow - 1) * 8 + (activeLampCol);
    }
    debug('ACTIVE_LAMP', this.activeLamp, activeLampRow, activeLampCol);
    //TODO commit after vblank??
  }
  ...
  write(offset, value) {
    ...
    switch (offset) {
      ...
      case OP.WPC_LAMP_ROW_OUTPUT:
        debug('WRITE', REVERSEOP[offset], value);
        this._updateActiveLamp(value, this.ram[OP.WPC_LAMP_COL_STROBE]);
        break;

      case OP.WPC_LAMP_COL_STROBE:
        debug('WRITE', REVERSEOP[offset], value);
        this._updateActiveLamp(this.ram[OP.WPC_LAMP_ROW_OUTPUT], value);
        break;
      ...
      case OP.WPC_ZEROCROSS_IRQ_CLEAR:
        ...
        if (timerEnabled !== this.periodicIRQTimerEnabled) {
          ...
          console.log('WRITE WPC_ZEROCROSS_IRQ_CLEAR periodic timer changed', timerEnabled);
        }
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_EXTBOARD3:
        ...
        case OP.WPC_LAMP_ROW_OUTPUT:
        case OP.WPC_LAMP_COL_STROBE:
        console.log('IIKEREAD!!!!', REVERSEOP[offset], this.ram[offset]);
        return this.ram[offset];

/*
      case OP.WPC_SWITCH_ROW_SELECT:
        debug('READ', REVERSEOP[offset], this.ram[OP.WPC_SWITCH_COL_SELECT], this.ram[OP.WPC_SWITCH_ROW_SELECT]);
        return this.ram[OP.WPC_SWITCH_ROW_SELECT];*/

      case OP.WPC_SWITCH_CABINET_INPUT:
        debug('READ AND CLEAR', REVERSEOP[offset], this.ram[offset]);
        temp = this.ram[offset];
        //TODO manually clear?
        //this.ram[offset] = 0x00;
        return temp;
      ...
      case OP.WPC_SWITCH_ROW_SELECT:
        debug('READ', REVERSEOP[offset], this.ram[offset]);
        //TODO
        return this.ram[OP.WPC_SWITCH_COL_SELECT];

      case OP.WPC_SHIFTADDRH:
        temp = this.ram[OP.WPC_SHIFTADDRH] + ((this.ram[OP.WPC_SHIFTADDRL] + (this.ram[OP.WPC_SHIFTBIT] >>> 3)) >>> 8);
      ...
      case OP.WPC_SHIFTBIT:
        ...
        return 1 << (this.ram[OP.WPC_SHIFTBIT] & 0x07);
      case OP.WPC_SHIFTBIT2:
        debug('READ WPC_SHIFTBIT2');
        return 1 << (this.ram[OP.WPC_SHIFTBIT2] & 0x07);
      ...
      case OP.WPC_ZEROCROSS_IRQ_CLEAR:
        temp = this.zeroCrossFlag << 7 | (this.ram[OP.WPC_ZEROCROSS_IRQ_CLEAR] & 0x7F);
        debug('READ WPC_ZEROCROSS_IRQ_CLEAR', temp, this.zeroCrossFlag ? 'ZCF_SET' : 'ZCF_NOTSET');
        this.zeroCrossFlag = 0;
        return temp;

      case OP.WPC_PERIPHERAL_TIMER_FIRQ_CLEAR:
        debug('READ WPC_PERIPHERAL_TIMER_FIRQ_CLEAR', this._irqSourceDmd);
        return this._irqSourceDmd === true ? 0x80 : 0x00;
...
$3fe9     Byte     WPC_SW_ROW_INPUT, R: Switch row read - unclear
$3fea     Byte     WPC_SW_COL_STROBE, W: Switch column enable - unclear
