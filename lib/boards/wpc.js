...
class WpcIo {
  ...
  _updateActiveLamp() {
    ...
    if (!this.activeLampRow || !this.activeLampCol) {
      this.activeLamp = 0;
    } else {
      this.activeLamp = (this.activeLampRow - 1) * 8 + (this.activeLampCol);
    }
    debug('ACTIVE_LAMP', this.activeLamp, this.activeLampRow, this.activeLampCol);
  ...
  write(offset, value) {
    ...
    switch (offset) {
      ...
      case OP.WPC_LAMP_ROW_OUTPUT:
        debug('WRITE', REVERSEOP[offset], value);
        this.activeLampRow = bitmagic.findMsbBit(value);
        this._updateActiveLamp();
        break;

      case OP.WPC_LAMP_COL_STROBE:
        debug('WRITE', REVERSEOP[offset], value);
        this.activeLampCol = bitmagic.findMsbBit(value);
        this._updateActiveLamp();
        break;
