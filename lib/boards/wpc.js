...
class WpcIo {
  ...
  getUiState() {
    ...
    const lampRow = this.genericState[OP.WPC_LAMP_ROW_OUTPUT];
    const lampColumn = this.genericState[OP.WPC_LAMP_COL_STROBE];
    const switchRow = this.genericState[OP.WPC_SWITCH_ROW_READ];
    const switchColumn = this.genericState[OP.WPC_SWITCH_COL_SELECT];
  ...
  setCabinetInput(value) {
    ...
    this.genericState[OP.WPC_SW_CABINET_INPUT] = value & 0xff;
  }
  ...
  read(offset) {
    ...
    switch (offset) {
      ...
      case OP.WPC_SW_CABINET_INPUT:
        debug('READ AND CLEAR', REVERSEOP[offset], this.genericState[offset]);
        console.log('READ AND CLEAR', REVERSEOP[offset], this.genericState[offset]);
        var result = this.genericState[offset];
        this.genericState[offset] = 0x00;
        return result;

      case OP.WPC_ROM_BANK:
        debug('READ', REVERSEOP[offset], this.genericState[offset]);
        return this.genericState[offset] & this.pageMask;

      case OP.WPC_SW_ROWREAD:
        //TODO
        return this.genericState[OP.WPC_SWITCH_COL_SELECT];
      ...
      case OP.WPC_ZEROCROSS_IRQ_CLEAR:
        var result = this.zeroCrossFlag << 7 | (this.genericState[OP.WPC_ZEROCROSS_IRQ_CLEAR] & 0x7F);
        debug('READ WPC_ZEROCROSS_IRQ_CLEAR', result, this.zeroCrossFlag);
