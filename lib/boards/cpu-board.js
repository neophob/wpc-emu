...
class WpcCpuBoard {
  ...
  modifyMemory(offset, value, block) {
    debug('modifyMemory', { offset, value, block });
    this.ram[offset] = value;
    if (block) {
      debug('PERSISTENT MEMORY CHANGE', { offset, value });
      this.memoryPatch.addPatch(offset, value, true);
    } else {
      this.memoryPatch.removePatch(offset);
    }
  }
