...
const MAXIMAL_STEPS_TO_RUN = 2000000 * 10;
...
class WpcCpuBoard {
  ...
  setDirectInput(offset, uint8Value) {
    this.asic.setDirectInput(offset, uint8Value);
  }
  ...
  //Synchronise up to target ticks
  executeToCycle(targetTicks) {
    const currentTicks = this.cpu.tickCount;
    const ticksToRun = targetTicks - currentTicks;
    console.log('executeToCycle', {ticksToRun, currentTicks});
    if (ticksToRun > 0 && ticksToRun < MAXIMAL_STEPS_TO_RUN) {
      return this.executeCycle(ticksToRun, 512);
    }
    console.log('RESET NEEDED!', ticksToRun);
    return 0;
  }
