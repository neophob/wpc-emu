...
class WpcCpuBoard {
  ...
  _bankswitchedRead(offset) {
    ...
    if (Asic.SYSTEM_ROM_BANK_NUMBER.includes(activeBank)) {
      debug('bankswitched read from system rom', activeBank, offset);
      return this.systemRom[offset];
    }
    const ofs = offset + activeBank * ROM_BANK_SIZE;
    if (ofs > this.gameRom.length) {
      console.log('bankswitched read from game rom %o', { activeBank, offset, pageOffset: ofs, data: this.gameRom[ofs], romlength: this.gameRom.length });
    }
    const pageOffset = (offset + activeBank * ROM_BANK_SIZE);// % this.gameRom.length;
