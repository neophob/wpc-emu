...
class WpcCpuBoard {
  ...
  _bankswitchedRead(offset) {
    ...
    if (Asic.SYSTEM_ROM_BANK_NUMBERS.includes(activeBank)) {
      const a = Asic.SYSTEM_ROM_BANK_NUMBERS.indexOf(activeBank) * 16 * 1024 - 1;
      debug('bankswitched read from system rom', { activeBank, offset, a });
      return this.systemRom[offset + a];
    }
