...
const CpuBoardAsic = require('./asic.js');
// TODO remove me
const SoundBoard = require('./sound-board.js');
// TODO move to asic
const ExternalIoBoard = require('./externalio-board.js');
// TODO move to asic
...
class WpcAsic {
  ...
  constructor(romObject) {
    this.ram = new Uint8Array(memoryMapper.MEMORY_ADDR_HARDWARE);
    ...
    this.externalIo = ExternalIoBoard.getInstance();
  ...
  reset() {
    ...
    this.ticksUpdateDmd = 0;
    this.protectedMemoryWriteAttempts = 0;
    //don't clear ram to imcrease reset time!
    //this.ram.fill(0);
    this.cpu.reset();
    this.cpuBoardAsic.reset();
    this.dmdBoard.reset();
  ...
  executeCycle(ticksToRun, tickSteps) {
    ...
    while (ticksExecuted < ticksToRun) {
      ...
      if (this.ticksIrq >= timing.CALL_IRQ_AFTER_TICKS) {
        ...
        //if (this.cpuBoardAsic.periodicIRQTimerEnabled) {
          this.cpu.irq();
        //}
      }
  ...
  _write8(offset, value) {
    ...
    switch (address.subsystem) {
      ...
      case memoryMapper.SUBSYSTEM_RAM:
        if (this.cpuBoardAsic.isMemoryProtectionEnabled() ||
          (offset & this.cpuBoardAsic.memoryProtectionMask) !== this.cpuBoardAsic.memoryProtectionMask) {
  ...
  _hardwareWrite(offset, value) {
    ...
    switch (address.subsystem) {
      ...
      case hardwareMapper.SUBSYSTEM_EXPANSION:
        console.log('EXPANSION_WRITE_NOT_IMPL', { offset, value });
        break;
      default:
  ...
  _hardwareRead(offset) {
    ...
    switch (address.subsystem) {
      ...
      case hardwareMapper.SUBSYSTEM_EXPANSION:
        console.log('EXPANSION_READ_NOT_IMPL', { offset });
        return 0;
      default:
        console.log(JSON.stringify(address));
