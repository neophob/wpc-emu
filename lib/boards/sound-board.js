 ...
 * @param {String} initObject.interruptCallback firq callback function
 * @param {Boolean} preDcsBoard preDCS boards use 8bit to communicate with the CPU board, DCS and later use 16 bit
 * @return {ListBuilder} factory methods to build list
 * @example
 *  listbuilder.buildBrowseList({
  *      title: 'list title',
...
class SoundBoard {
  ...
  constructor(initObject, preDcsBoard) {
    this.interruptCallback = initObject.interruptCallback;
    this.preDcsBoard = preDcsBoard;
    this.volume = 0;
