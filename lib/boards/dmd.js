...
class DMD {
  ...
  copyScanline() {
    ...
    if (this.scanline === 0) {
      ...
      if (Number.isInteger(this.nextActivePage)) {
        ...
        console.log('select active page',this.activePage);
        this.nextActivePage = undefined;
      }
    }
    ...
    if (this.requestFIRQ === true) {
      if (this.scanline === this.ram[OP.WPC_DMD_SCANLINE] & 0x1F) {
        console.log('FIRQ_REQUESTED', this.scanline, this.ram[OP.WPC_DMD_SCANLINE]);
  ...
  read(offset) {
    ...
    switch (offset) {
      case OP.WPC_DMD_SCANLINE:
        console.log('READ', REVERSEOP[offset], this.highPage);
