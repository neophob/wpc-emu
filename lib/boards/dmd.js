...
const DMD_PAGE_LOW_ADDRESS = 0x3A00;
const DMD_PAGE_HIGH_ADDRESS = 0x3FBC;
...
function getInstance(interruptCallback, ram) {
  return new DMD(interruptCallback, ram);
}

class DMD {

  // dmd has its own 8kb video ram to hold 16 pages of video frames
  constructor(interruptCallback, ram) {
  ...
  getUiState() {
    const page1 = this.videoRam.slice(0, DMD_PAGE_SIZE);
    const page2 = this.videoRam.slice(1 * DMD_PAGE_SIZE, 2 * DMD_PAGE_SIZE);
    const page3 = this.videoRam.slice(2 * DMD_PAGE_SIZE, 3 * DMD_PAGE_SIZE);
    const page4 = this.videoRam.slice(3 * DMD_PAGE_SIZE, 4 * DMD_PAGE_SIZE);
  ...
  getPageOffset(offset, isLowPage) {
    if (isLowPage) {
      const deltaLow = offset - DMD_PAGE_LOW_ADDRESS;
      return this.lowPage * DMD_PAGE_SIZE + deltaLow;
    }
    const deltaHigh = offset - DMD_PAGE_HIGH_ADDRESS;
  ...
  write(offset, value) {
    ...
    switch (offset) {
      ...
      case OP.WPC_DMD_LOW_PAGE:
        //TODO not sure here, i guess low page can be max 8
        temp = value & 0xf;
        this.hardwareRam[offset] = temp;
        this.lowPage = temp;
        debug('WRITE', REVERSEOP[offset], this.hardwareRam[offset]);
        console.log('WRITE', REVERSEOP[offset], this.hardwareRam[offset], this.getPageOffset(offset, true));
        break;
      ...
      case OP.WPC_DMD_HIGH_PAGE:
        ...
        this.hardwareRam[offset] = temp;
        this.highPage = temp;
        console.log('WRITE', REVERSEOP[offset], this.hardwareRam[offset]);
        debug('WRITE', REVERSEOP[offset], this.hardwareRam[offset]);
        break;
